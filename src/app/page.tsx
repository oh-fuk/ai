...
import {
  BookOpen, Brain, FileText, Dices, Clock,
  Music, Mic, Calculator, Map, Globe,
  BarChart3, PenTool, Calendar, CheckCircle,
  ChevronRight, Play, X, Menu,
  Sparkles, Zap, Infinity, Users, Volume2,
  FileSearch, LineChart, FileCheck, Sigma,
  BookCopy, SpellCheck, MessageCircle,
} from "lucide-react";
...
export default function LandingPage() {
  ...
  const features = [
    ...
    { icon: FileText, title: "Paper Generator", description: "Generate full practice exam papers with MCQs, short & long questions.", color: "from-green-500 to-emerald-500" },
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] text-white overflow-x-hidden">
      ...
      {/* ── Nav ── */}
      <motion.nav
        className={`fixed top-0 w-full z-40 transition-all duration-300 ${scrolled ? "bg-black/50 backdrop-blur-xl border-b border-white/10" : "bg-transparent"
          }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <motion.div className="flex items-center gap-3" whileHover={{ scale: 1.05 }}>
              ...
              <span className="text-2xl font-bold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent font-headline">
                AthenaAI
              </span>
            </motion.div>
            ...
            <div className="hidden md:flex items-center gap-4">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link href="/login" className="px-6 py-2.5 rounded-full border border-white/20 text-white hover:bg-white/10 transition-all inline-block">
                  Log In
                </Link>
              </motion.div>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link href="/register" className="px-6 py-2.5 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-medium shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all inline-block">
                  Sign Up Free
                </Link>
              </motion.div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <motion.div whileHover={{ scale: 1.05, boxShadow: "0 0 40px rgba(6,182,212,0.4)" }} whileTap={{ scale: 0.95 }}>
                <Link href="/register" className="px-8 py-4 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-lg shadow-lg shadow-cyan-500/30 flex items-center gap-2 inline-flex">
                  Get Started Free <ChevronRight className="w-5 h-5" />
                </Link>
              </motion.div>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link href="/login" className="px-8 py-4 rounded-full border border-white/20 text-white font-medium text-lg backdrop-blur-sm hover:bg-white/5 transition-all flex items-center gap-2 inline-flex">
                  Log In
                </Link>
              </motion.div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <p className="text-gray-400 text-lg max-w-2xl mx-auto">
              Everything you need to excel in your studies, powered by Gemini AI
            </p>
          </motion.div>
        ...
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-xl text-gray-400 mb-10 max-w-2xl mx-auto">
              Join students who are already studying smarter with AthenaAI. Start for free today.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <motion.div whileHover={{ scale: 1.05, boxShadow: "0 0 60px rgba(6,182,212,0.5)" }} whileTap={{ scale: 0.95 }}>
                <Link href="/register" className="px-10 py-5 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-xl shadow-xl shadow-cyan-500/30 inline-block">
                  Start Free
                </Link>
              </motion.div>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link href="/login" className="px-10 py-5 rounded-full border border-white/20 text-white font-medium text-xl hover:bg-white/5 transition-all inline-block">
                  Log In
                </Link>
              </motion.div>
            </div>
